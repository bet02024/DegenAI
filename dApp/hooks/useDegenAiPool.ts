...
export function useDegenAiPool() {
  ...
  const { data: userShares, refetch: refetchUserShares } = useReadContract({
    address: currentContracts?.DEGENAI,
    abi: CONTRACTS.DEGENAI.abi,
  ...
  const { data: usdcAllowanceData, refetch: refetchUsdcAllowance } = useReadContract({
    ...
    args: address && currentContracts ? [address, currentContracts.DEGENAI] : undefined,
    query: {
      enabled: !!address && !!isChainSupported && !!currentContracts,
      retry: 3,
      retryDelay: 1000,
  ...
  const approveUsdc = useCallback(
    async (amount: string) => {
      ...
      writeContract({
        ...
        args: [currentContracts.DEGENAI, amountWei],
      })
    },
